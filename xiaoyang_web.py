"""
XiaoYang Studio web client — session đăng nhập (cookie), không Kling / API v1 key.

Luồng giống trình duyệt:
  login → /api/upload → POST /api/tasks → poll → GET /api/tasks/{id}/file
"""
...
SESSION_FILE = Path(__file__).resolve().parent / get_env("XIAOYANG_SESSION_FILE", "xiaoyang_session.json")
...
class XiaoyangWebClient:
    """Gọi API nội bộ web xiaoyang.online qua cookie session."""
    ...
    def _load_cookies(self) -> None:
        if not SESSION_FILE.is_file():
            return
        try:
            for item in json.loads(SESSION_FILE.read_text(encoding="utf-8")):
    ...
    def _request(self, method: str, path: str, **kwargs) -> dict:
        ...
        if r.status_code == 401:
            raise XiaoyangAuthError("Chưa đăng nhập hoặc session hết hạn")
        try:
    ...
    def login(self, email: str | None = None, password: str | None = None) -> dict:
        ...
        if not email or not password:
            raise XiaoyangAuthError("Thiếu XIAOYANG_EMAIL / XIAOYANG_PASSWORD trong .env")
        data = self._request(
            "POST",
            "/api/auth/login",
    ...
    def create_motion_task(
        ...
        *,
        ...
        if modal_key not in (MOTION_V26, MOTION_V30):
            raise ValueError(f"modal_key không hợp lệ: {modal_key}")
        body = {
            "modal_key": modal_key,
    ...
    def refresh_task(self, task_id: str) -> dict:
    ...
    def get_task(self, task_id: str) -> dict:
        try:
            return self.refresh_task(task_id)
    ...
    def download_task_file(self, task_id: str, dest_path: str) -> str:
        ...
        url = self._url(f"/api/tasks/{task_id}/file")
        with self.session.get(url, stream=True, timeout=int(get_env("XIAOYANG_DOWNLOAD_TIMEOUT_SEC", "600"))) as r:
            if r.status_code == 401:
            ...
            if not r.ok:
                try:
                    ...
                    detail = r.text[:300]
                raise XiaoyangWebError(f"Download HTTP {r.status_code}: {detail}")
