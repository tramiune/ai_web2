...
import { aggregateStats, formatVnd, formatNumber } from './admin-stats.js';
...
function renderVideoUrlPreview(containerId, url, options = {}) {
    ...
    probe.onloadedmetadata = () => {
        if (probe.duration > MAX_REFERENCE_VIDEO_SEC + 0.15) {
            showToast(t('modals.video_duration_limit'));
            container.innerHTML = '';
            syncUploadZonePreviewState(container);
            return;
        }
        mountUrlVideoPreview(container, url, options);
    };
...
function renderVideoFilePreview(containerId, file, options = {}) {
    ...
    probe.onloadedmetadata = () => {
        ...
        if (duration > MAX_REFERENCE_VIDEO_SEC + 0.15) {
            showToast(t('modals.video_duration_limit'));
            if (options.inputId) {
                const input = document.getElementById(options.inputId);
                if (input) input.value = '';
...
window.switchAdminTab = (tabName) => {
    ...
    const adminControls = document.querySelector('#admin-panel .admin-controls');
    if (adminControls) adminControls.style.display = tabName === 'stats' ? 'none' : '';
...
let adminActiveTab = 'orders';            // 'orders' | 'topups' | 'users' | 'referrals' | 'bots' | 'stats'
let adminStatsPreset = 'today';
let adminStatsCharts = { orders: null, engine: null, topups: null };
let adminStatsUiReady = false;
const ADMIN_STATS_SCAN_LIMIT = 4000;
...
function refreshActiveAdminSubscription() {
    ...
    if (adminActiveTab === 'orders') {
        ...
        fbUnsub('adminBots');
        fbUnsub('adminRenderProvider');
        setupAdminStatsUiOnce();
        loadAdminStats();
    }
}

function vnWeekStartDateString(now = new Date()) {
    const fmt = new Intl.DateTimeFormat('en-CA', { timeZone: VN_TIMEZONE });
    for (let i = 0; i < 7; i++) {
        const d = new Date(now);
        d.setDate(d.getDate() - i);
        const wd = new Intl.DateTimeFormat('en-US', { timeZone: VN_TIMEZONE, weekday: 'short' }).format(d);
        if (wd === 'Mon') return fmt.format(d);
    }
    return fmt.format(now);
}

function adminStatsRangeFromPreset(preset) {
    const today = getVnDateString();
    if (preset === 'today') return { from: today, to: today };
    if (preset === '7d') {
        const d = new Date();
        d.setDate(d.getDate() - 6);
        return { from: getVnDateString(d), to: today };
    }
    if (preset === '30d') {
        const d = new Date();
    ...
    if (preset === 'week') return { from: vnWeekStartDateString(), to: today };
    if (preset === 'month') {
...
function setupAdminStatsUiOnce() {
    if (adminStatsUiReady) return;
    adminStatsUiReady = true;

    document.querySelectorAll('.admin-stats-preset').forEach((btn) => {
        btn.addEventListener('click', () => {
            document.querySelectorAll('.admin-stats-preset').forEach((b) => b.classList.remove('active'));
            btn.classList.add('active');
            adminStatsPreset = btn.dataset.preset || 'today';
            const range = adminStatsRangeFromPreset(adminStatsPreset);
            const fromEl = document.getElementById('admin-stats-from');
            const toEl = document.getElementById('admin-stats-to');
    ...
    const refreshBtn = document.getElementById('admin-stats-refresh-btn');
    if (refreshBtn) {
    ...
    const range = adminStatsRangeFromPreset('today');
    const fromEl = document.getElementById('admin-stats-from');
    const toEl = document.getElementById('admin-stats-to');
    if (fromEl) fromEl.value = range.from;
    if (toEl) toEl.value = range.to;
...
function destroyAdminStatsCharts() {
    Object.keys(adminStatsCharts).forEach((key) => {
        if (adminStatsCharts[key]) {
            adminStatsCharts[key].destroy();
            adminStatsCharts[key] = null;
        }
    });
}

function renderAdminStatsCharts(result) {
    if (typeof Chart === 'undefined') return;
    destroyAdminStatsCharts();

    const labels = result.daily.map((d) => d.date);
    const completed = result.daily.map((d) => d.ordersCompleted);
    const failed = result.daily.map((d) => d.ordersFailed);
    const topupVnd = result.daily.map((d) => d.topupVnd);
    const topupUsers = result.daily.map((d) => d.topupUsers);

    const ordersCanvas = document.getElementById('admin-stats-chart-orders');
    if (ordersCanvas) {
        adminStatsCharts.orders = new Chart(ordersCanvas, {
            type: 'bar',
            data: {
                labels,
                datasets: [
                    { label: t('admin.stats_kpi_completed'), data: completed, backgroundColor: 'rgba(74, 222, 128, 0.7)' },
                    { label: t('admin.stats_kpi_failed'), data: failed, backgroundColor: 'rgba(248, 113, 113, 0.7)' },
                ],
            },
            options: {
                responsive: true,
    ...
    const tTotals = result.totals.byEngine;
    const engineLabels = [
        'XY fast', 'XY 2K', 'AD fast', 'AD 2K', 'VAE fast', 'VAE 2K',
    ];
    const engineData = [
        tTotals.xiaoyang.fast, tTotals.xiaoyang.turbo,
        tTotals.aidancing.fast, tTotals.aidancing.turbo,
        tTotals.videoaieasy.fast, tTotals.videoaieasy.turbo,
    ];
    const engineCanvas = document.getElementById('admin-stats-chart-engine');
    if (engineCanvas) {
        adminStatsCharts.engine = new Chart(engineCanvas, {
            type: 'bar',
            data: {
                labels: engineLabels,
                datasets: [{
                    label: t('admin.stats_chart_engine'),
                    data: engineData,
                    backgroundColor: [
                        '#a78bfa', '#c4b5fd', '#4ade80', '#86efac', '#fbbf24', '#fde68a',
                    ],
                }],
            },
            options: {
    ...
    const topupsCanvas = document.getElementById('admin-stats-chart-topups');
    if (topupsCanvas) {
        adminStatsCharts.topups = new Chart(topupsCanvas, {
            type: 'line',
            data: {
                labels,
                datasets: [
                    {
                        label: t('admin.stats_kpi_topup_vnd'),
                        data: topupVnd,
                        borderColor: '#60a5fa',
                    ...
                    {
                        label: t('admin.stats_kpi_topup_users'),
                        data: topupUsers,
                        borderColor: '#fbbf24',
...
function renderAdminStatsTable(result) {
    const body = document.getElementById('admin-stats-table-body');
    if (!body) return;
    if (!result.daily.length) {
        body.innerHTML = `<tr><td colspan="7" style="text-align:center; opacity:0.5; padding:2rem;">—</td></tr>`;
        return;
    }
    body.innerHTML = result.daily.map((row) => `
        <tr>
            <td>${row.date}</td>
            <td>${formatNumber(row.topupUsers)}</td>
            <td>${formatNumber(row.topupCount)}</td>
            <td>${formatVnd(row.topupVnd)}</td>
            <td>${formatNumber(row.ordersCompleted)}</td>
            <td>${formatNumber(row.ordersFailed)}</td>
            <td>${formatVnd(row.renderCostVnd)}</td>
        </tr>
    `).join('');
}

function renderAdminStatsKpis(result) {
    const t = result.totals;
    const gross = t.topupVnd - t.renderCostVnd;
    const set = (id, val) => {
        const el = document.getElementById(id);
        if (el) el.textContent = val;
    };
    set('stat-kpi-topup-users', formatNumber(t.topupUsers));
    set('stat-kpi-topup-vnd', formatVnd(t.topupVnd));
    set('stat-kpi-completed', formatNumber(t.ordersCompleted));
    set('stat-kpi-failed', formatNumber(t.ordersFailed));
    set('stat-kpi-render-cost', formatVnd(t.renderCostVnd));
    set('stat-kpi-gross', formatVnd(gross));
}

async function loadAdminStats() {
    if (!window.__isAdmin || adminActiveTab !== 'stats') return;

    const fromEl = document.getElementById('admin-stats-from');
    const toEl = document.getElementById('admin-stats-to');
    const from = fromEl?.value || getVnDateString();
    ...
    const warnEl = document.getElementById('admin-stats-warning');
    const rangeEl = document.getElementById('admin-stats-range-label');
    if (loadingEl) loadingEl.style.display = 'block';
    if (warnEl) { warnEl.style.display = 'none'; warnEl.textContent = ''; }
    if (rangeEl) rangeEl.textContent = t('admin.stats_range', { from: range.from, to: range.to });

    try {
        const { db, collection, query, orderBy, getDocs, limit } = window.firebase;
        const [ordersSnap, topupsSnap] = await Promise.all([
            getDocs(query(collection(db, 'orders'), orderBy('createdAt', 'desc'), limit(ADMIN_STATS_SCAN_LIMIT))),
            getDocs(query(collection(db, 'topups'), orderBy('createdAt', 'desc'), limit(ADMIN_STATS_SCAN_LIMIT))),
        ]);

        const orders = ordersSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        const topups = topupsSnap.docs.map((d) => ({ id: d.id, ...d.data() }));

        const truncated = ordersSnap.size >= ADMIN_STATS_SCAN_LIMIT || topupsSnap.size >= ADMIN_STATS_SCAN_LIMIT;
        if (truncated && warnEl) {
            warnEl.style.display = 'block';
            warnEl.textContent = t('admin.stats_truncated', { limit: ADMIN_STATS_SCAN_LIMIT });
        }

        const result = aggregateStats(orders, topups, range);
        renderAdminStatsKpis(result);
        renderAdminStatsTable(result);
        renderAdminStatsCharts(result);
    } catch (err) {
        console.error('Admin stats error:', err);
        showToast(t('admin.stats_load_error', { msg: err.message }));
    } finally {
